...
Standard genetic association studies assume additive effects, where each copy of an allele contributes equally to a phenotype. However, many genetic effects are non-additive -- for example, recessive variants only manifest when two copies are present. `arcade` provides a suite of tools to:
...
> **Note**: If running via Docker, prefix commands with `docker run -v $PWD:/data fhlassen/arcade`.
