...
int main(int argc, char *argv[]) {
  ...
  arcade::printHeader("COUNT_BY_GENE", "Count genotypes by gene", files,
                          params);
  ...
  arcade::HtsFileUPtr vcfFile(bcf_open(pathInput.c_str(), "r"));
  if (!vcfFile) {
  ...
  arcade::BcfHdrUPtr header(bcf_hdr_read(vcfFile.get()));
  if (!header) {
    std::cerr << "Error: Cannot read header from VCF file." << std::endl;
    return 1;
  }

  arcade::Bcf1UPtr record(bcf_init());
  if (!record) {
