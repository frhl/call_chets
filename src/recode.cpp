...
// sortChromosomes is now provided by cli_utils.hpp as arcade::sortChromosomes
...
// readGroupMap is now provided by cli_utils.hpp as arcade::readGroupMap
...
using arcade::Bcf1UPtr;
using arcade::BcfHdrUPtr;
using arcade::HtsFileUPtr;
using arcade::openVcf;
...
int main(int argc, char *argv[]) {
  ...
  if (!scaleByGroupPath.empty()) {
    std::string errorMsg;
    groupMap = arcade::readGroupMap(scaleByGroupPath, errorMsg);
    if (!errorMsg.empty()) {
  ...
  if (needTwoPass) {
    std::vector<std::string> sortedContigs = arcade::sortChromosomes(chromosomes);
    bcf_close(fp);
