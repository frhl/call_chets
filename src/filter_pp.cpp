...
int main(int argc, char *argv[]) {
  ...
  arcade::printHeader("FILTER_PP", "Filter VCF by PP threshold", files,
                          params);

  // Use RAII for input file and header
  arcade::HtsFileUPtr inFile(bcf_open(inputPath.c_str(), "r"));
  if (!inFile) {
    std::cerr << "Could not open input file: " << inputPath << std::endl;
    return 1;
  }

  arcade::BcfHdrUPtr hdr(bcf_hdr_read(inFile.get()));
  if (!hdr) {
  ...
  arcade::HtsFileUPtr outFile(bcf_open(outputFilePath.c_str(), outMode));
  if (!outFile) {
